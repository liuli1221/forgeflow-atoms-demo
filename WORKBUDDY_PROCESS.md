...
本文记录使用 WorkBuddy 完成 ForgeFlow（Atoms-like Demo）的最终实现过程。

文档只描述当前有效方案：**所有用户创建与修改请求统一进入真实 AI 生成链路，确定性代码只负责计划审批、安全校验、自动修复、版本和持久化护栏。** 不保留已经废弃的生成路线和阶段性实现。

- 项目目录：仓库根目录 `atomlite-workbuddy/`
...
最终验收目标不是“根据关键词展示几个固定模板”，而是：

1. 计算器、贪吃蛇、任务管理器以及开放式需求都进入同一条 AI 生成入口；
2. 生成结果包含完整的 `index.html`、`styles.css`、`app.js`，可以真实交互；
3. 生成结果必须经过确定性校验，不合格时自动修复，只有通过校验的版本才能成为 READY；
4. Builder 状态、生成源码、版本记录和生成应用的业务数据在刷新后仍然存在；
5. 在线地址可以直接体验，不要求访问者提供 API Key；
6. 服务端密钥不进入浏览器、Git 仓库或导出文件；
7. 产品界面不展示底层供应商和模型名称，对外统一使用“AI Agent”。
...
这些限制均在实现和文档中明确呈现，没有用本地模板回退来伪装 AI 生成成功。

---

## 12. 面试演示建议

1. 打开 Production URL，展示欢迎页和三栏工作台；
2. 新建项目，输入“生成一个现代风格计算器，支持加减乘除、清空和连续计算”；
3. 展示计划卡，强调批准前不会生成；
4. 批准后观察真实运行轨迹、校验结果和 READY 版本；
5. 在预览中点击 `7 + 5 =`，确认结果为 `12`；
6. 切到代码页，展示三个完整源码文件；
7. 输入修改要求，生成 v2；
8. 在版本页恢复 v1；
9. 再生成一个任务管理器，新增数据后刷新页面，证明业务数据持久化；
10. 说明架构边界：AI 负责按需求生成，确定性代码负责安全、可用性、版本和持久化。
